%Compute vertical tail mass moment of inertia.
function I = vTailInertia(Tail_HalfSpan, Tail_Chord, Tail_Density, Tail_XLoc, Xcg)
...
% calculations done for half tail
Ixxw = sum(Mw(2, :).^2.*Mw(3, :));
Izzw = sum(Mw(1, :).^2.*Mw(3, :));
Iyyw = sum((Mw(1, :).^2 + Mw(2, :).^2).*Mw(3, :));
