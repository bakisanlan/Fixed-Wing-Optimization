%Add static stability constraints to optimization problem.
...
%Add the helper functions to the MATLAB path.
addpath('AerodynamicCoefficients');
addpath('MassMomentofInertia');
%Aerodynamic Coefficients used by FixedWing object
%To create an Aero.FixedWing object, calculate all 21 aerodynamic 
% coefficients that define the aircraft's dynamics response.
%Set the options to specify that the output is a 3-element row vector and
% turn on analysis and evaluation reuse to speed up the optimization.
...
%Use the component build-up method to calculate the mass moment of intertia
% of the aircraft, which is also needed to create the Aero.FixedWing object.
% Each function calculates the 3-by-3 inertia matrix for a component with 
% respect to the aircraft's center of gravity. Then, the matrices are added 
% together.
%Set the options to specify that the output is a 3-by-3 matrix, and turn 
% on analysis and evaluation reuse to speed up the optimization.
...
Ip = fcn2optimexpr(@payloadInertia, ...
    payload.XLoc, payload.Boxed.SandMass, payload.Boxed.EmptyMass, ...
    payload.Boxed.Height, payload.Boxed.SandHeight, payload.Boxed.Length, ...
    fuselage.SideLength, aircraft.Xcg, ...
    options{:});

% Fuselage Inertia Matrix
If = fcn2optimexpr(@fuselageInertia, ...
    fuselage.Mass, fuselage.SideLength, fuselage.LengthBody, ...
...
inertiaMatrix = fcn2optimexpr(@aircraftInertia, ...
    ...
    options{:});
%Stability Derivatives using FixedWing object
%Use the staticStability function to compute all 48 static stability 
% derivatives of the aircraft. Only 10 of them have a noticeable effect 
% on the aircraft's stability, reducing the overall number of constraints.
